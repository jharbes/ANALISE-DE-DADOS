...
#mcorr = precos %>% as.data.frame() %>% select(date,symbol,adjusted)  %>% spread(symbol, adjusted)
})
...
#prices = filters %>%  tq_get(get = "stock.prices", from = "2012-01-01")
...
eventReactive(input$go,{
  precos = precos()
  corrplot(cor(precos), method ='number')
